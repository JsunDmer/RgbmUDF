...
class treenode:
    def __init__(self,treeContent,factorDict,factorSplit,id,status=0):
        ...
        if self.splitVar in factorDict.keys():
            ...
            self.splitCodePred = [ h.strip() for h in self.splitCodePred ]
        else:
...
def printTree(trr,span,typ):
    if trr.status<>-1: 
        print '\t' * span + '├── %s=%s,splitName=%s,value=%s\n' %(typ,trr.id,inred(trr.splitVar,42),trr.splitCodePred)    
...
if __name__ == "__main__":
    try:  
        opts,args = getopt.getopt(sys.argv[1:], "hi:n:t:d:", ["help", "input=","name=","type=","nodeid="])
        for o,v in opts:  
            if o in ("-h", "--help"):
                usage()
                sys.exit(1);
    ...
    if typ == 1:
        ...
        if nid == None:
           ...
           treeContent = s.createTreeNode(int(nid)) 
           trr = s.GetTree(treeContent,0)
           printTree(trr,1,"rootNode")
